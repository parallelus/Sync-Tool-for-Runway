<br><div class="nav-tabs nav-tabs-second">
	...
	<a href="<?php echo $this->self_url('import-plugins'); ?>&action=connection&connection_id=<?php echo $connection_id; ?>&operation=import&what=plugins" class="nav-tab nav-tab-active"><?php echo __('Import', 'framework'); ?></a>
</div>
...
<div class="postbox">
	...
	<h3 class="hndle"><span><?php echo __('Server Plugins', 'framework'); ?></span></h3>
	<div class="inside" >
		<table class="wp-list-table widefat">
			<thead>
				<tr>
					...
					<th id="action" class="manage-column column-name" width="70"><?php echo __('Action', 'framework'); ?></th>
				</tr>
			</thead>
			<tbody id="the-list">		
						...
						<td class="column-description desc">
							<?php
								if(isset($local_plugins[$key])){
									echo __("Installed", 'framework');
								}
								else{
									$plugin = explode('/', $key);
									$plugin = str_replace('.php', '', $plugin[0]);
									?>
									<a href="<?php echo admin_url('plugin-install.php?tab=plugin-information&plugin='.$plugin.'&TB_iframe=true&width=640&height=359'); ?>" class="thickbox" title="More information about Akismet 2.5.7"><?php echo __('Details', 'framework'); ?></a><br>
									<?php		
									if ( current_user_can('install_plugins') ){
										$install_now_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $plugin), 'install-plugin_' . $plugin);
										?>
										<a href="<?php echo "$install_now_url"; ?>" target="_blank"><?php echo __('Install now', 'framework'); ?></a><br>
										<a href="<?php echo "http://wordpress.org/extend/plugins/$plugin"; ?>" target="_blank"><?php echo __('Download', 'framework'); ?></a>
										<?php
									}																
								}
							 ?>
